...
작성일: 2026-09-21
분석 기준: [22번 노트북](../22_mert_unseen_generator.ipynb)의 최초 저장 결과
...
설정의 기준은 기존 최적화 binary 실험의 선택 체크포인트 `mert_best.joblib`이다. 체크포인트는 용량 및 배포 정책에 따라 Git에서 제외했다. 과거 17번 실험의 layer 9 결과와 구분하여, 해당 체크포인트에 저장된 다음 설정을 읽었다.
...
기존 RBF-SVM과 Log-Mel CNN은 저장된 [SVM 결과](../results/unseen_generator/unseen_generator_metrics_all.csv), [CNN 결과](../results/cnn_unseen_generator/cnn_unseen_generator_metrics_all.csv)를 읽었다. 세 모델의 Test Track ID가 완전히 같은지도 예측 파일로 확인했다.
...
2026-09-21 재실행에서도 최초 결과와 동일한 MERT 지표와 데이터 수가 기록됐다. 중복 실행 폴더는 Git에서 제외했다.
...
이 절은 앞의 MusicGen/Udio holdout과 **다른 실험**이다. 기존 Test의 **539 Track(REAL 45, FAKE 494)**과 **1,572 Segment(REAL 135, FAKE 1,437)**를 모두 사용한다. `original`은 데이터셋에 저장된 원래 파일을 뜻하며 무압축 오디오를 뜻하지 않는다. `mp3_128`, `mp3_64`는 [12번 실험의 전사 기록](../results/mp3_robustness/transcode_report.csv)에 있는 동일 곡의 128/64 kbps 재인코딩본이다.

세 조건 모두 MP3 실험용 MERT 캐시에서 같은 Test Segment ID와 순서를 사용한다. 대용량 배열은 Git에서 제외하고 제출 패키지에는 행 대응 index와 메타데이터를 포함했다. MP3 파일을 곡 처음부터 디코딩하고 원래 Segment 시작점에서 자른 뒤, 패딩 없이 MERT 유효 hidden frame의 평균을 저장한 캐시다. 압축 전후 변화량은 같은 방식으로 추출된 **MP3 실험용 `clean` 캐시**를 기준으로 계산했다. 이 캐시는 일반 binary Test 캐시와 디코딩 방식이 달라 값이 미세하게 다르다.
...
| 파일 | 내용 |
|---|---|
| [22번 노트북](../22_mert_unseen_generator.ipynb) | 실행 코드, 한국어 단계별 설명, 셀별 결과 요약 |
| [데이터 수](../results/mert_unseen_generator/mert_unseen_dataset_counts.csv) | holdout 필터 전·후 Track·Segment·원곡 그룹 수 |
| [MERT 지표](../results/mert_unseen_generator/mert_unseen_metrics.csv) | Segment·Track Test 지표와 Validation 임계값 |
| [세 모델 비교](../results/mert_unseen_generator/mert_unseen_comparison.csv) | 동일 Test Track에 대한 SVM·CNN·MERT 결과 |
| [Track 예측](../results/mert_unseen_generator/mert_unseen_predictions_track.csv) / [Segment 예측](../results/mert_unseen_generator/mert_unseen_predictions_segment.csv) | 곡·구간별 점수와 고정 임계값의 예측 |
| [설정 및 QC](../results/mert_unseen_generator/mert_unseen_config.json) | 체크포인트 출처, 캐시, LR 설정, holdout별 임계값, 16개 QC 결과 |
| [MERT MP3 지표](../results/mert_mp3_robustness/mert_mp3_metrics.csv) / [세 모델 MP3 비교](../results/mert_mp3_robustness/mert_mp3_track_comparison.csv) | Original·128/64 kbps의 Segment·Track 지표와 SVM/CNN 비교 |
| [MP3 AUC 변화량](../results/mert_mp3_robustness/mert_mp3_auc_change.csv) | 같은 모델의 Original 대비 압축 조건 변화량 |
| [MP3 Track 예측](../results/mert_mp3_robustness/mert_mp3_predictions_track.csv) / [Segment 예측](../results/mert_mp3_robustness/mert_mp3_predictions_segment.csv) | 조건별 점수, 고정 임계값, 예측 |
| [MP3 설정 및 QC](../results/mert_mp3_robustness/mert_mp3_config.json) | 캐시·체크포인트 출처, 임계값, 10개 QC 결과 |

노트북을 처음부터 다시 실행하면 기존 결과를 덮어쓰지 않고 각 결과 폴더의 `runs/` 아래 새 실행 폴더를 만든다. 최신 핵심 수치는 [최종 분석 보고서](FINAL_REPORT.md)에도 반영했으며, 이 문서는 22번 실험의 설정과 검증 내용을 더 자세히 기록한다.
