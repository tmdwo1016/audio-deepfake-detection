...
> 제출본 안내: 이 문서는 프로젝트 저장소의 개요를 제출 폴더 구조에 맞게 정리한 사본이다. 최신 통합 수치와 포함 파일은 [`../SUMMARY.md`](../SUMMARY.md)와 [`../README.md`](../README.md)를 기준으로 한다.
...
> 분석 완료일: 2026-09-22
> 상세 결과: [프로젝트 요약](../SUMMARY.md) · [최종 통합 보고서](FINAL_REPORT.md) · [MERT 후속 분석](22_mert_unseen_generator_report.md) · [변경사항](CHANGELOG.md)
...
CNN은 clean 조건에서는 우수하지만 압축 후 하락 폭이 더 컸고, MERT의 AUC 감소가 가장 작았습니다. 높은 in-domain 성능만으로 codec 강건성을 판단하기는 어렵습니다.
...
위 표에 적힌 파일이 각 단계의 실행본입니다. 20번 PCA/UMAP 분석은 주 실행본과 v2 보존 사본을 모두 포함했습니다.

## 저장소 구조

```text
.
├── notebooks/                    # 순차 분석 노트북 24개
├── src/                          # FMA 다운로드·추출 보조 스크립트
├── data/                         # 전처리 데이터와 cache index
├── summary/                      # 제출용 통합 결과 CSV
...
- MERT가 두 unseen-generator holdout에서 가장 높은 AUC를 보였지만, 평가 범위는 두 생성기로 제한됩니다.
...
최신 해석 범위와 제한사항은 [프로젝트 요약](../SUMMARY.md)에 정리했습니다. 기존 통합 보고서는 [MERT 확장 이전 보고서](FINAL_REPORT_pre_MERT_extensions.md)로 함께 보존했습니다.
