...
각 holdout generator의 FAKE를 Train과 Validation에서 제거하고, Test REAL 전체와 해당 generator의 FAKE만으로 평가했습니다. Holdout별 scaler·LR 가중치와 임계값은 대상 생성기를 제외한 자료로 다시 학습·선택했습니다. 다만 layer 4와 C=0.01은 전체 생성기 binary Validation에서 미리 선택된 설정이므로, 설정 선택까지 완전히 생성기 독립적인 실험은 아닙니다.
...
MERT가 두 holdout 모두에서 가장 높은 AUC를 보였습니다. 하지만 평가 대상은 MusicGen과 Udio뿐이며 신뢰구간이나 유의성 검정을 계산하지 않았으므로, 모든 새로운 생성기에 대한 일반화 우위로 해석하지 않습니다.

![Unseen generator comparison](../results/mert_unseen_generator/figures/unseen_generator_track_auc_mert_comparison.png)
...
| MERT+LR | Original | **0.9845** | **0.0738** | **0.9363** |
| MERT+LR | MP3 128 | **0.9851** | **0.0708** | **0.9444** |
| MERT+LR | MP3 64 | **0.9605** | **0.1102** | **0.8485** |

Original 대비 64 kbps AUC 하락은 SVM 0.0583, CNN 0.1516, MERT 0.0240입니다. MERT의 순위 분리력은 가장 안정적이었지만 REAL 오탐률은 0.0889에서 0.2889로 증가했습니다. 따라서 AUC뿐 아니라 오류 방향도 함께 봐야 합니다. MP3 표의 MERT는 layer 4 valid-frame/full-decode cache를 사용해 in-domain layer 9 결과와 Original AUC가 0.9845와 0.9872로 다릅니다.

![Codec robustness comparison](../results/mert_mp3_robustness/figures/mert_mp3_track_auc_comparison.png)
...
flowchart TD
    ...
    B -->|Unseen generator| C[MERT가 두 holdout에서 최고 AUC]
    B -->|MP3 codec| D[MERT AUC 하락폭 최소]
...
2. **MERT의 미노출 생성기 결과가 가장 안정적이었습니다.** 다만 MusicGen과 Udio 두 조건만 평가했으므로 적용 범위는 제한적입니다.
...
- MERT holdout의 layer와 C는 과거 전체 생성기 Validation에서 선택되어, 설정 선택까지 완전히 holdout-independent하지는 않습니다.
...
2. MP3 외 AAC, Opus, streaming normalization, resampling, noise 조건 확장
3. Codec augmentation 및 domain-adversarial training 비교
4. Generator와 genre/source를 분리하는 invariant representation 학습
5. Bootstrap confidence interval과 source-level 통계 검정 추가
6. 시간·주파수 attribution으로 구체적인 generator artifact 위치 분석

## 11. 결과 파일 안내

- Detection: [`results/baseline/`](../results/baseline/), [`results/cnn/`](../results/cnn/), [`results/mert/`](../results/mert/)
- Unseen generator: [`results/mert_unseen_generator/`](../results/mert_unseen_generator/)
- Codec robustness: [`results/mert_mp3_robustness/`](../results/mert_mp3_robustness/)
- Generator attribution: [`results/generator_attribution/`](../results/generator_attribution/)
- PCA/UMAP: [`results/generator_fingerprint_visualization/`](../results/generator_fingerprint_visualization/)
- Genre/feature analysis: [`results/generator_genre_feature_analysis/`](../results/generator_genre_feature_analysis/)
- 1차 통합 표와 오류 사례: [`results/final_analysis/`](../results/final_analysis/)
- MERT 강건성 상세 설명: [`docs/22_mert_unseen_generator_report.md`](22_mert_unseen_generator_report.md)
